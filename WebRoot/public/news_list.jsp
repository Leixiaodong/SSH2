...
<div class="main1">
  <table class="list_tab" width="80%" align="center" cellspacing="1" bordercolor="#44AEDB"  cellpadding="1" border="1">
				  ...
				  <tr>
				    <td colspan="2" align="center" bgcolor="#6A82DA"><span class="span3">新闻列表</span></td>
			      </tr>
			    <s:iterator value="#request.newsList" var="news" status="s">  
				  <tr>
				    <td width="21%" align="right" bgcolor="#E4EDF9">新闻标题：</td>
...
<c:if test="${pageNow == 1}">
   		 ...
   		 <a href="<%=basePath%>news!showList?s_pageNow=2">下一页&nbsp;&nbsp;</a> 
   		 <a href="<%=basePath%>news!showList?s_pageNow=${pageCount}">末页</a> </p>
  </c:if>
      	<c:if test="${pageNow == pageCount}">
      		<p style="font-size:18px font-weight:bold;margin-left:240px; margin-bottom:80px;" align="left"> 当前页数:[${pageNow }/${pageCount }]&nbsp;
      		 <a href="<%=basePath%>news!showList?s_pageNow=1">首页</a>
      			<a href="<%=basePath%>news!showList?s_pageNow=${pageNow-1}">&nbsp;&nbsp;上一页</a></p>
      	</c:if>
      	<c:if test="${pageNow < pageCount && pageNow > 1}">
      		<p style="font-size:18px font-weight:bold;margin-left:240px; margin-bottom:80px;" align="left"> 当前页数:[${pageNow}/${pageCount }]&nbsp;&nbsp; 
      		<a href="<%=basePath%>news!showList?s_pageNow=1">首页</a>
      			<a href="<%=basePath%>news!showList?s_pageNow=${pageNow-1}">&nbsp;&nbsp;上一页</a>
      		<a href="<%=basePath%>news!showList?s_pageNow=${pageNow+1}">&nbsp;&nbsp;下一页</a> 
      		<a href="<%=basePath%>news!showList?s_pageNow=${pageCount}">&nbsp;&nbsp;末页</a></p>
      	</c:if>	
