...
<jsp:include page="head.jsp" />
  ...
  <table class="list_tab" align="center" width="80%" cellspacing="1" bordercolor="#44AEDB"  cellpadding="1" border="1">
				  ...
				  <tr>
				    <td colspan="2" align="center" bgcolor="#6A82DA"><span class="span3">学生列表</span></td>
			      </tr>
			      <s:iterator value="#request.studentList" var="student" status="s">  
				  <tr>
				    <td width="15%" align="right" bgcolor="#E4EDF9">学生姓名：</td>
				    <td width="85%" bgcolor="#E4EDF9"><s:property value="#student.name"/></td>
			      </tr>
			      
				  <tr>
				    <td align="right" bgcolor="#F1F3F5">性别：</td>
				    <td bgcolor="#F1F3F5"><s:property value="#student.sex"/></td>
			      </tr>
				  <tr>
				    <td align="right" bgcolor="#E4EDF9">年级：</td>
				    <td bgcolor="#E4EDF9"><s:property value="#student.grade.grade"/></td>
			      </tr>
				  <tr>
...
<c:if test="${pageNow == 1}">
   		 ...
   		 <a href="<%=basePath%>student!showList?s_pageNow=2">下一页&nbsp;&nbsp;</a> 
   		 <a href="<%=basePath%>student!showList?s_pageNow=${pageCount}">末页</a> </p>
  </c:if>
      	<c:if test="${pageNow == pageCount}">
      		<p style="font-size:18px font-weight:bold;margin-left:240px;margin-bottom:80px;" align="left"> 当前页数:[${pageNow }/${pageCount }]&nbsp;
      		 <a href="<%=basePath%>student!showList?s_pageNow=1">首页</a>
      			<a href="<%=basePath%>student!showList?s_pageNow=${pageNow-1}">&nbsp;&nbsp;上一页</a></p>
      	</c:if>
      	<c:if test="${pageNow < pageCount && pageNow > 1}">
      		<p style="font-size:18px font-weight:bold;margin-left:240px;margin-bottom:80px;" align="left"> 当前页数:[${pageNow}/${pageCount }]&nbsp;&nbsp; 
      		<a href="<%=basePath%>student!showList?s_pageNow=1">首页</a>
      			<a href="<%=basePath%>student!showList?s_pageNow=${pageNow-1}">&nbsp;&nbsp;上一页</a>
      		<a href="<%=basePath%>student!showList?s_pageNow=${pageNow+1}">&nbsp;&nbsp;下一页</a> 
      		<a href="<%=basePath%>student!showList?s_pageNow=${pageCount}">&nbsp;&nbsp;末页</a></p>
      	</c:if>	
