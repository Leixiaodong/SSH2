	...
	<body>
		...
		<div class="main1">
             ...
             <table class="list_tab" align="center" width="80%" cellspacing="1" bordercolor="#44AEDB"  cellpadding="1" border="1">
				  ...
				  <tr>
				    <td colspan="3" align="center" bgcolor="#6A82DA"><span class="span3">教师列表</span></td>
			      </tr>
                    <s:iterator value="#request.teacherList" var="teacher" status="s">
                  
                   <tr>
				    <td align="right" bgcolor="#F1F3F5">教师编号：</td>
				    <td width="54%" bgcolor="#F1F3F5"><s:property value="#teacher.id" /></td>
				    <td width="29%" bgcolor="#F1F3F5"><a href="<%=basePath%>admin/Teacher_updateInput.action?id=${teacher.id}" target="_self">【编辑】</a>
				    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="<%=basePath%>admin/Teacher_delete.action?id=${teacher.id}" target="_self" onclick="delComit()">【删除】</a></td>
		          </tr>
				  <tr>
...
<c:if test="${pageNow == 1}">
   		 ...
   		 <a href="<%=basePath%>admin/Teacher_showList?s_pageNow=2">下一页&nbsp;&nbsp;</a> 
   		 <a href="<%=basePath%>admin/Teacher_showList?s_pageNow=${pageCount}">末页</a> </p>
  </c:if>
      	<c:if test="${pageNow == pageCount}">
      		<p style="font-size:18px font-weight:bold;margin-left:240px;margin-bottom:80px;" align="left"> 当前页数:[${pageNow }/${pageCount }]&nbsp;
      		 <a href="<%=basePath%>admin/Teacher_showList?s_pageNow=1">首页</a>
      			<a href="<%=basePath%>admin/Teacher_showList?s_pageNow=${pageNow-1}">&nbsp;&nbsp;上一页</a></p>
      	</c:if>
      	<c:if test="${pageNow < pageCount && pageNow > 1}">
      		<p style="font-size:18px font-weight:bold;margin-left:240px;margin-bottom:80px;" align="left"> 当前页数:[${pageNow}/${pageCount }]&nbsp;&nbsp; 
      		<a href="<%=basePath%>admin/Teacher_showList?s_pageNow=1">首页</a>
      			<a href="<%=basePath%>admin/Teacher_showList?s_pageNow=${pageNow-1}">&nbsp;&nbsp;上一页</a>
      		<a href="<%=basePath%>admin/Teacher_showList?s_pageNow=${pageNow+1}">&nbsp;&nbsp;下一页</a> 
      		<a href="<%=basePath%>admin/Teacher_showList?s_pageNow=${pageCount}">&nbsp;&nbsp;末页</a></p>
      	</c:if>	
